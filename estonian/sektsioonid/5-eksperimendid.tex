...
Mudel & Maht (lk) & Kirjavigade arv & Kirjavigu lk kohta \\
Gemini 3.1 Pro (Antigravity) & 39 & 28 & 0,72 \\
Claude Sonnet 4.6 (Cowork) & 40 & 13 & 0,33 \\
Claude Opus 4.6 (Cowork) & 61 & 5 & 0,08 \\
GPT-5.4 (Codex) & 47 & 1 & 0,02 \\
\end{tblr}
\end{table}
...
Claude Sonnet 4.6 töös leidus ortograafiliselt problemaatilisi sõnu rohkem ning need olid mustriliselt mitmekesised. Kõige nähtavamal kohal asus tiitellehe pealkiri \textit{Suurkeelemudelite kasutamine automaatseskoodiülevaatuses}, kus kahe sõna vahelt puudus tühik (sisemise abstrakti pealkiri oli korrektne). Korduvad topelttähed avaldusid sõnades \textit{deviatsiooonid} (kolm \textit{o}-d), \textit{annotatsiooonid} (neli \textit{o}-d) ja \textit{genereeeritud} (kolm \textit{e}-d). Lühitähed olid puudu sõnades \textit{Eeltrenimine} (õige \textit{Eeltreenimine}), \textit{kategoriad} (õige \textit{kategooriad}) ja \textit{kodile treenitud} (õige \textit{koodile}). Sõnaõpetuslikud või tõlkevaldkonna eksimused esinesid mittesõnades \textit{kaksikeliigsust} (mõeldud oli \textit{topeltkodeerimine}), \textit{ristemoodaalset} (õige \textit{ristmodaalset}), \textit{saldestamata} (mõeldud oli \textit{saneerimata}) ning kokkukleebisõnas \textit{inglinter-rater} (puuduv tühik lühendi \textit{ingl} ja võõrtermini \textit{inter-rater} vahelt). Üksikute näpukatena täheldati ka \textit{turvaauuke} (õige \textit{turvaauke}), \textit{suuresel} (õige \textit{suurel}) ja \textit{kasumlikkusest} (õige \textit{kasulikkusest}). Osa vigadest asus eriti nähtaval kohal -- näiteks tiitellehel -- mis vähendab töö esmamuljet.
