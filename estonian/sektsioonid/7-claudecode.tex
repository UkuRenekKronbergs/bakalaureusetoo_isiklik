...
\begin{itemize}
    ...
    \item \textbf{Ära usalda ühe kasutajaviibega (\textit{single-prompt}) tulemust pimesi ilma kriitilise ülevaatamiseta.} Käesoleva uurimuse eksperimendis (vt peatükk~\ref{subsec:cowork-juhtumiuuring}) näitas ühe kasutajaviibega lähenemine juba üllatavalt tugevaid tulemusi (eriti Opus 4.6 + Cowork kombinatsiooni puhul -- 61 lehekülge, 5 kirjaviga, 0 esimese isiku asesõna), kuid sellegipoolest sisaldas töö hallutsineeritud viiteid ja mõningaid sisulisi probleeme. Parima kvaliteedi saavutamiseks on mõistlik kombineerida ühe viibega genereerimist iteratiivsete paranduste ja kasutaja järelevalvega.
    ...
    \item \textbf{Kasuta tipptasemel mudelit nii palju kui võimalik.} Käesoleva töö kirjutamise ajal (märts 2026) oli Claude perekonna võimekaim mudel Claude Opus 4.6 ja see sobis kõige paremini terviklike peatükkide ja keeruliste akadeemiliste tekstide genereerimiseks, kuigi selle hind oli oluliselt kõrgem kui Sonnet mudelil. Kuna Anthropic on vahepeal avaldanud uuemaid versioone, tasub soovituse rakendamisel kontrollida, milline Opus-tasemel mudel on parajasti kättesaadav.
\end{itemize}
...
Eriti tähelepanuväärne on Codex laienduse tulemus GPT-5.4 mudeliga. Käesoleva uurimuse eksperimendis (vt peatükk~\ref{subsec:exp-tervik}) keeldus GPT-5.4 tavapärase ChatGPT vestlusliidese kaudu terviklikku lõputööd genereerimast, viidates akadeemilise aususe piirangutele, ning andis selle asemel 8-leheküljelise ingliskeelse juhendi. Codex laienduse kaudu suutis aga sama mudel -- täpselt sama ühe kasutajaviibega -- genereerida visuaalselt teiste kombinatsioonide väljunditega sarnase tervikliku dokumendi. See erinevus ei ole otseses mõttes ``ohutuspiirangu ületamine'', vaid näitab, et agentne töövoog muudab ülesande konteksti ja tööjaotust -- liides, süsteemijuhised ja ohutuspoliitika on Codexi puhul teistsugused kui ChatGPT vestlusliideses, ning ülesanne jaotub väiksemateks faili- ja kompileerimispõhisteks sammudeks. Selle tulemusena käitus sama mudel erinevas liideses sisuliselt erinevalt; seda ei tohi käsitleda kui juhist, kuidas turvakoolitusest mööda minna.
...
\begin{itemize}
    ...
    \item \emph{väited} selle kohta, et rakendus läbis 149 automatiseeritud testi (47 krüptograafia ühiktesti + 61 serveri ühiktesti + 41 integratsioonitesti);
    \item \emph{väide} OWASP Top 10 vastavuse analüüsi kohta;
    \item \emph{väide} süsteemi kasutatavuse skaala (SUS) hindamisest skooriga 77,0;
    \item \emph{väide}, et turvaauditi tulemusel ei tuvastatud ühtegi kriitilist turvaauku.
\end{itemize}

Olulise metoodilise märkusena: neid teste, auditit ega kasutajauuringut käesoleva uurimuse autor iseseisvalt ei teostanud -- tegemist on kombinatsiooni genereeritud empiiriliste väidetega, mis kuuluvad samasse kategooriasse viitehallutsinatsioonidega ning tuleb inimese poolt eraldi kontrollida.

\subsubsection{Järeldused juhtumiuuringust}

Juhtumiuuring kinnitas, et Opus 4.6 + Cowork kombinatsioon suudab selles katses genereerida tervikliku, struktureeritud ja tehniliselt detailse lõputöö. Genereeritud töö vastas TÜ ATI vorminõuetele ja sisaldas kõiki nõutud peatükke. Peamised tugevused olid:
\begin{itemize}
    \item Tehniline sügavus ja detailsus, mis ületas teiste kombinatsioonide tulemusi;
...
Peamised puudused olid endiselt seotud viidete hallutsineerimisega (ca 15\% viidetest vajasid kontrolli), kombinatsiooni enda genereeritud empiiriliste väidetega (149 testi, SUS skoor 77,0 jms) ning mõningate eesti keele terminoloogia ebajärjekindlustega.
...
Töö algetapis teostati sama ühe kasutajaviibega katse, mida rakendati peaeksperimendis, et kaardistada võimalikku struktuuri ja teemat. Katse eesmärk oli näha, kuidas mudel ise tõlgendab lühikest juhist ning milliseid peatükke ja alapeatükke ta oodatavasse struktuuri paigutab. Katse tulemusi kasutati üksnes sisukorra ja teemade kaardistamiseks -- katse väljund aitas autoril paremini mõista, milliseid teemasid käsitleda ja kuidas materjali järjestada. Selle kaardistuse põhjal kirjutas autor peatükid uuesti ise, tuginedes loetud kirjandusele ja oma eksperimendi tulemustele; algset mustandteksti ei kopeeritud käesolevasse tööse. Juhul kui sõnastuslikke kattuvusi üksikutes üldistes sissejuhatavates lausetes esineb, on need autori poolt üle kontrollitud ja ümber sõnastatud, et töö ei kasutaks otseselt TI-genereeritud algteksti ilma deklareerimiseta. Kasuliku kõrvalväljundina andis katse autorile ka esmase hinnangu selle kohta, milliseid peatükke TI ise oluliseks peab (sissejuhatus, taust, metoodika, mudelite võrdlus, kokkuvõte) ning milliste peatükkide (nt arutelu, eetika, piirangud) kohta tuli autoril iseseisvalt juurde mõelda.
...
\begin{itemize}
    ...
    \item \textbf{Kulud.} Kuigi Claude Code'i tellimus on üliõpilase eelarve kontekstis tuntav kulutus, jäi see oluliselt madalamaks kui professionaalse toimetamise teenuse maksumus (vt peatükk~\ref{subsec:hinnakujundus}). Tööga seotud TI-sessioonid jaotusid ühtlaselt kogu kirjutamisperioodile, mis võimaldas kulusid paremini ennustada ja hallata. Läbipaistvuse huvides tuleb märkida, et autor kulutas kogu uurimisperioodi jooksul TI-teenuste tellimuste peale ligikaudu 360~€ omavahendeid. See summa kattis järgmised tellimused: kaks kuud Claude Max'i (5x variant), üks kuu ChatGPT Pro'd, üks kuu ChatGPT Plus'i, kolm kuud Gemini AI tasulist plaani ning kaks kuud Perplexity tasulist plaani. Tellimused hõlmasid nii peaeksperimendi läbiviimist (nelja mudel-tööriista kombinatsiooni testimine) kui ka käesoleva töö enda kirjutamise tööriistu, mistõttu summat ei saa tervikuna omistada üksnes lõputöö kirjutamisele ega üksnes eksperimendile.
\end{itemize}
